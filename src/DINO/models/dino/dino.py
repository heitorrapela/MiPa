...
from .modality_invariance_wrapper import FeatureInvarianceTrainer
...
class DINO(nn.Module):
    ...
    def detection_forward(self, samples: NestedTensor, features, poss, targets:List=None):
        srcs = []
        masks = []
    ...
    def backbone_forward(self, samples: NestedTensor, targets:List=None):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        features, poss = self.backbone_forward(samples)

        out = self.detection_forward(samples, features, poss, targets)
...
class SetCriterion(nn.Module):
    ...
    def loss_modality_invariance(self, outputs, targets, indices, num_boxes):
        losses = {'loss_modality_invariance' : torch.tensor(0)}
        if 'target_modality_map' in outputs:
            output_modality_map = outputs['output_modality_map']
            target_modality_map = outputs['target_modality_map']

            if output_modality_map is None or target_modality_map is None:
                return losses
            
            # Balancing
            size, m_sum = len(output_modality_map.view(-1)), sum(target_modality_map.view(-1)).item()
            w1 = m_sum / size
            w2 = 1 - w1
            pos_weight = torch.tensor([w2/w1], device=target_modality_map.device)

            loss_mod_inv = F.binary_cross_entropy_with_logits(output_modality_map.unsqueeze(-1),
                                                              target_modality_map.unsqueeze(-1),
                                                              reduction='mean', )
                                                            #   weight=pos_weight)

            losses['loss_modality_invariance'] = loss_mod_inv
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            'modality_invariance': self.loss_modality_invariance,
        }
    ...
    def forward(self, outputs, targets, return_indices=False):
        ...
        if self.training and dn_meta and 'output_known_lbs_bboxes' in dn_meta:
            ...
            for loss in self.losses:
                ...
                if 'labels' in loss:
                    kwargs = {'log': False}
                if 'modality_invariance' in loss:
                    continue
                    # l_dict.update(self.get_loss(loss, outputs, targets, dn_pos_idx, num_boxes*scalar,**kwargs))
            ...
            l_dict['loss_modality_invariance'] = torch.as_tensor(0.).to('cuda')
            losses.update(l_dict)
            ...
            for idx, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    if loss == 'labels':
                        ...
                        kwargs = {'log': False}
                    if loss == 'modality_invariance':
                        continue
                        # l_dict = self.get_loss(loss, outputs, None, None, None)
                    else:
                ...
                if self.training and dn_meta and 'output_known_lbs_bboxes' in dn_meta:
                    ...
                    for loss in self.losses:
                        ...
                        if loss == 'modality_invariance':
                            continue
                            l_dict.update(self.get_loss(loss, outputs, None, None, None))
                        else:
...
def build_dino(args):
    ...
    if args.modality_invariance_training:
        losses += ['modality_invariance']
        weight_dict['loss_modality_invariance'] = 1.0
    if args.masks:
        losses += ["masks"]
