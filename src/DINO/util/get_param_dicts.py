...
def get_param_dict(args, model_without_ddp: nn.Module):
    ...
    assert param_dict_type in ['default', 'ddetr_in_mmdet', 'large_wd', 'domain_invariance']
    ...
    # by default
    if param_dict_type == 'domain_invariance':
        param_dicts = [
            {"params": [p for n, p in model_without_ddp.named_parameters() if "backbone" not in n and "domain_classifier" not in n and p.requires_grad]},
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "backbone" in n and p.requires_grad],
                "lr": args.lr_backbone,
            },
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "domain_classifier" in n and p.requires_grad],
                "lr": 8*args.lr_dom_inv,
            }
        ]
        return param_dicts
